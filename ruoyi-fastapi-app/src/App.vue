...
function checkLogin() {
  ...
  // H5 端允许某些页面免登录直达（例如软件库列表/详情）
  const h5Path = getH5EntryPath();
  if (isPublicH5Path(h5Path)) return;

  proxy.$tab.reLaunch("/pages/login");
}

function getH5EntryPath() {
  if (typeof window === "undefined") return "";
  const hash = window.location.hash || "";
  // e.g. "#/pages/software/index?x=1" -> "/pages/software/index"
  const path = hash.startsWith("#") ? hash.slice(1) : hash;
  return (path || "/").split("?")[0];
}

function isPublicH5Path(path) {
