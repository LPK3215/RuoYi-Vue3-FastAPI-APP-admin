...
class LogAggregatorService:
    ...
    # Redis>=6.2 supports XAUTOCLAIM. For older Redis (e.g. 5.x on Windows),
    # calling XAUTOCLAIM will raise "unknown command" and spam logs.
    _xautoclaim_supported: bool | None = None
    ...
    async def _claim_pending(cls, redis: aioredis.Redis, consumer_name: str) -> None:
        ...
        if LogConfig.log_stream_claim_idle_ms <= 0:
            return
        if cls._xautoclaim_supported is False:
            return
        start_id = '0-0'
        while True:
            try:
                ...
                if cls._xautoclaim_supported is None:
                    cls._xautoclaim_supported = True
            except Exception as exc:
                msg = str(exc)
                if cls._xautoclaim_supported is None and 'unknown command' in msg and 'XAUTOCLAIM' in msg.upper():
                    cls._xautoclaim_supported = False
                    logger.warning(
                        '当前 Redis 版本不支持 XAUTOCLAIM，已跳过 pending 认领逻辑（建议升级到 Redis >= 6.2）'
                    )
                    return
                raise
            if not result:
