...
def test_portal_categories(http: requests.Session) -> None:
    resp = http.get(f"{Config.backend_url}/portal/software/categories", timeout=10)
...
def test_portal_list_pagination_and_keyword(http: requests.Session) -> None:
    ...
    resp_kw = http.get(
        f"{Config.backend_url}/portal/software/list",
        params={"pageNum": 1, "pageSize": 10, "keyword": "Python"},
        timeout=10,
    )
    ...
    resp_os = http.get(
        f"{Config.backend_url}/portal/software/list",
        params={"pageNum": 1, "pageSize": 10, "openSource": "1"},
        timeout=10,
    )
    ...
    resp_tag = http.get(
        f"{Config.backend_url}/portal/software/list",
        params={"pageNum": 1, "pageSize": 10, "tag": "python"},
        timeout=10,
    )
    tag_data = _assert_ok_json(resp_tag)
    assert any(x.get("softwareId") == 20001 for x in tag_data.get("rows", [])), tag_data


def test_portal_facets(http: requests.Session) -> None:
    resp = http.get(f"{Config.backend_url}/portal/software/facets", timeout=10)
    data = _assert_ok_json(resp)
...
def test_portal_detail_contains_downloads(http: requests.Session) -> None:
    ...
    resp = http.get(f"{Config.backend_url}/portal/software/20001", timeout=10)
    data = _assert_ok_json(resp)
    detail = data.get("data") or {}
    assert detail.get("softwareId") == 20001
...
def test_admin_category_and_software_crud(http: requests.Session, admin_token: str) -> None:
    ...
    resp = http.post(f"{Config.backend_url}/tool/software/category", json=cat_payload, headers=headers, timeout=10)
    _assert_ok_json(resp)

    # 2) find category id
    resp = http.get(
        f"{Config.backend_url}/tool/software/category/list",
        params={"pageNum": 1, "pageSize": 10, "categoryName": cat_name},
        headers=headers,
        timeout=10,
    )
    data = _assert_ok_json(resp)
    ...
    resp = http.post(f"{Config.backend_url}/tool/software/item", json=sw_payload, headers=headers, timeout=10)
    _assert_ok_json(resp)
    ...
    deny = resp.json()
    assert deny.get("code") != 200, deny

    # 5) find software id and detail
    resp = http.get(
        f"{Config.backend_url}/tool/software/item/list",
        params={"pageNum": 1, "pageSize": 10, "softwareName": sw_name},
        headers=headers,
        timeout=10,
    )
    data = _assert_ok_json(resp)
    ...
    resp = http.get(f"{Config.backend_url}/tool/software/item/{software_id}", headers=headers, timeout=10)
    ...
    resp = http.get(f"{Config.backend_url}/tool/software/item/facets", headers=headers, timeout=10)
    ...
    resp = http.get(
        f"{Config.backend_url}/tool/software/item/list",
        params={"pageNum": 1, "pageSize": 10, "tag": "test", "platform": "web"},
        headers=headers,
        timeout=10,
    )
    ...
    # 6) change publish status -> off shelf and portal should not find it via keyword
    resp = http.put(
    ...
    resp = http.get(
        f"{Config.backend_url}/portal/software/list",
        params={"pageNum": 1, "pageSize": 10, "keyword": sw_name},
        timeout=10,
    )
    ...
    resp = http.delete(f"{Config.backend_url}/tool/software/item/{software_id}", headers=headers, timeout=10)
    _assert_ok_json(resp)

    resp = http.delete(f"{Config.backend_url}/tool/software/category/{category_id}", headers=headers, timeout=10)
    _assert_ok_json(resp)
